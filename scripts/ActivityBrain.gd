...
func tick(delta: float, walk_bounds: Rect2) -> ActivityDecision:
	_activity_timer += delta
	if _activity_timer < _duration[_current_activity]:
		return null
	_activity_timer = 0.0
	return _decide(walk_bounds)

func _decide(walk_bounds: Rect2) -> ActivityDecision:
	if _rest_timer >= _rest_threshold:
		_current_activity = Global.StateName.LAY
		_rest_timer = 0.0
	else:
		var total_weight := 0.0
		for weight in _preference.values():
			total_weight += weight

		var random_value := _rng.randf() * total_weight
		var cumulative_weight := 0.0
		for activity in _preference.keys():
			cumulative_weight += _preference[activity]
	...
	if _current_activity in [Global.StateName.WALK, Global.StateName.RUN, Global.StateName.SPRINT] and _rng.randf() < _walk_target_chance:
		walk_target = Vector2(
